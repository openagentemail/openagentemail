...
test('R5c real OAE boundary accepts HTTPS or loopback HTTP only and construction makes no network call', () => {
  ...
  for (const url of ['http://oae.example.test', 'ftp://oae.example.test', 'https://user:pass@oae.example.test', 'https://oae.example.test/#fragment', 'https://oae.example.test/?swallow=/v1/tasks', 'https://oae.example.test?', 'https://oae.example.test#', 'https://oae.example.test/?', 'https://oae.example.test/#']) assert.throws(() => safeOaeBaseUrl(url));
