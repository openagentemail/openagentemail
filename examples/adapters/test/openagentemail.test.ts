...
import { isValidTaskView, OaeClient, OaeHttpError } from '../src/openagentemail.js';
...
test('R5h TaskView validates the exact production ApprovalEvent union through create/get/list and rejects depth/shape confusion', async () => {
  ...
  const nested = (depth: number): unknown => depth === 1 ? 'leaf' : { child: nested(depth - 1) }; assert.equal(isValidTaskView(projected({ type: 'request', snapshot }, nested(10))), true); assert.equal(isValidTaskView(projected({ type: 'request', snapshot }, nested(11))), false); assert.equal(isValidTaskView(projected({ type: 'request', snapshot }, Number.POSITIVE_INFINITY)), false); assert.equal(isValidTaskView(projected({ type: 'request', snapshot }, Array.from({ length: 1_001 }, () => 0))), false); assert.equal(isValidTaskView(projected({ type: 'request', snapshot }, Object.fromEntries(Array.from({ length: 1_001 }, (_, index) => [`k${index}`, index])))), false);
});
