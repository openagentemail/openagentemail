...
const stateStore = new RunStateStore(directory); const correlationStore = new CorrelationStore(join(directory, 'correlation')); const taskStore = new FixtureJsonStore<OaeTask>(directory, 'oae-task.json'); const receiptStore = new RunStateStore(directory, 'tool-receipt.json');
...
if (mode === 'pause') {
  ...
  await new FixtureJsonStore(directory, 'pause-result.json').save({ taskId: awaiting.taskId, phase: awaiting.phase, approvals: 1, executions: graph.executions.count });
} else if (mode === 'decide') {
  ...
  const persisted = await stateStore.load(); const restored = await (await import('@openai/agents')).RunState.fromString(graph.agent, persisted); const receipt = JSON.parse(await receiptStore.load()) as { finalStateHash: string; executions: number }; await new FixtureJsonStore(directory, 'result.json').save({ decision, phase: result.phase, taskId: result.taskId, executions: receipt.executions, final: `${decision}-final`, pendingInterruptions: restored.getInterruptions().length, receipt: receipt.finalStateHash.length });
}
