...
const correlationStore = new CorrelationStore(join(directory, 'correlation')); const taskPath = join(directory, 'oae-task.json');
...
if (mode === 'pause') {
  const canonical = { requester, responder, subject, body: workflow }; const record = createIntent({ framework: 'langgraph', correlationId, operationKey: langGraphOperationKey(threadId), requestFingerprint: requestFingerprint(canonical), expectedParticipants: { requester, responder }, frameworkStateRef: 'langgraph-checkpoints.sqlite', approvalItemKey: langGraphApprovalKey(threadId), now: '2026-02-03T00:00:00.000Z' }); await correlationStore.save(record); const awaiting = await pauseLangGraph({ directory, threadId, workflow, record, correlationStore, oae }); await writeFile(join(directory, 'pause-result.json'), JSON.stringify({ phase: awaiting.phase, taskId: awaiting.taskId, threadId, operationKey: awaiting.operationKey, inputEvents: (await loadTask()).messages.filter((m) => m.state === 'input-required').length }), { mode: 0o600 });
} else if (mode === 'decide') {
  ...
  const record = await correlationStore.load(correlationId); const result = await resumeLangGraph({ directory, threadId, record, correlationStore, task: await loadTask() }); const final = await inspectLangGraphFinal(directory, threadId, result); const task = await loadTask(); await writeFile(join(directory, 'result.json'), JSON.stringify({ phase: result.phase, taskId: result.taskId, threadId, operationKey: result.operationKey, decision: final.decision, effects: final.effects, checkpointId: final.checkpointId, finalStateFingerprint: final.finalStateFingerprint, effectObservationFingerprint: final.effectObservationFingerprint, inputEvents: task.messages.filter((m) => m.state === 'input-required').length }), { mode: 0o600 });
}
