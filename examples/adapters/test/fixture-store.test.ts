...
test('R5d fixture store pre-rename failure preserves prior bytes, cleans its temporary, and reaches directory durability seam', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'fixture-store-crash-')); const target = join(directory, 'oae-task.json'); const initial = new FixtureJsonStore<{ value: number }>(directory, 'oae-task.json'); await initial.save({ value: 1 }); const failing = new FixtureJsonStore<{ value: number }>(directory, 'oae-task.json', { beforeRename: async () => { throw new Error('crash-before-rename'); } }); await assert.rejects(() => failing.save({ value: 2 }), /crash-before-rename/); assert.equal(await readFile(target, 'utf8'), JSON.stringify({ value: 1 })); assert.deepEqual((await readdir(directory)).filter((name) => name.startsWith('.oae-task.json.')), []);
  let directorySync = 0; const durable = new FixtureJsonStore<{ value: number }>(directory, 'oae-task.json', { beforeDirectorySync: async () => { directorySync += 1; } }); await durable.save({ value: 2 }); assert.equal(directorySync, 1); assert.deepEqual(await durable.load(), { value: 2 });
});
