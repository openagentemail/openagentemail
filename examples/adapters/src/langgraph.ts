...
class EffectSink {
  ...
  async execute(decision: { decision: 'approved' | 'rejected' }): Promise<EffectObservation> {
    if (decision.decision !== this.binding.decision) throw new CorrelationSafetyError('LangGraph effect decision contradicts authoritative evidence'); const ledgerStore = new JsonStore<EffectLedger>(this.directory, effectLedgerFilename(this.binding));
    if (decision.decision === 'rejected') { if (await ledgerStore.loadOptional()) throw new CorrelationSafetyError('LangGraph rejected decision has a protected effect ledger'); await this.checkpoint?.('before-rejection-observation'); const observation: RejectedEffectObservation = { version: 1, ...this.binding, effects: 0 }; const saved = await new JsonStore<EffectObservation>(this.directory, effectObservationFilename(this.binding)).createOrLoad(observation, (value) => validObservation(value, this.binding)); await this.checkpoint?.('after-rejection-observation'); return saved; }
