...
function validApprovalEvent(value: unknown): value is ApprovalEvent { if (!object(value) || typeof value.type !== 'string') return false; if (value.type === 'request') return exactKeys(value, ['type', 'snapshot'], ['type', 'snapshot']) && validApprovalSnapshot(value.snapshot); if (value.type === 'decision') return exactKeys(value, ['type', 'digest', 'decision'], ['type', 'digest', 'decision']) && typeof value.digest === 'string' && /^[a-f0-9]{64}$/i.test(value.digest) && (value.decision === 'approved' || value.decision === 'rejected'); return value.type === 'expired' && exactKeys(value, ['type', 'digest'], ['type', 'digest']) && typeof value.digest === 'string' && /^[a-f0-9]{64}$/i.test(value.digest); }
