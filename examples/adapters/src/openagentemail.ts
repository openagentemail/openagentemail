...
export function isValidTaskView(value: unknown): value is OaeTask { if (!object(value) || !exactKeys(value, ['id', 'from', 'to', 'subject', 'state', 'createdAt', 'updatedAt', 'messages', 'result', 'parentTaskId', 'kind', 'approval', 'claimedUntil', 'leaseGeneration', 'leaseStatus', 'expiryProjection'], ['id', 'from', 'to', 'subject', 'state', 'createdAt', 'updatedAt', 'messages'])) return false; const leasePair = (value.claimedUntil === undefined && value.leaseGeneration === undefined && value.leaseStatus === undefined) || (safeText(value.claimedUntil) && Number.isInteger(value.leaseGeneration) && (value.leaseGeneration as number) >= 1 && ((value.leaseStatus === undefined) || value.leaseStatus === 'disabled')); const expiryOk = value.expiryProjection === undefined || value.expiryProjection === 'past-deadline-unmaterialized'; return safeText(value.id) && safeText(value.from) && safeText(value.to) && safeText(value.subject) && safeText(value.createdAt) && safeText(value.updatedAt) && validState(value.state) && Array.isArray(value.messages) && value.messages.length <= 1_000 && value.messages.every(validMessage) && (value.parentTaskId === undefined || safeText(value.parentTaskId)) && (value.kind === undefined || value.kind === 'approval') && ((value.kind === 'approval') === (value.approval !== undefined)) && (value.approval === undefined || validApprovalSnapshot(value.approval)) && leasePair && expiryOk; }
