...
- **Tasks: 租约/状态突变兜底码改为 `task_operation_failed`** (#330)：`POST /v1/tasks/:id/{claim,lease,release,claim-lost,decision,state}` 六处与 UI task 突变（`taskMutationError`）在未命中已映射域码时，对外码由 `502 {error:"smtp_error"}` 改为 `502 {error:"task_operation_failed"}`（状态码与 body 形状不变；**已映射域码与状态码一律不变**；`POST /v1/tasks` create 段 pre-create `502 {error:"smtp_error"}` 不变）。这些路径不发 SMTP，旧码属语义漂移。客户端若按错误码判因需改判。**合并 ≠ 生效，生效于下次部署窗**。
