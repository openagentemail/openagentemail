...
- **API: 日志面发射路径收敛为唯一原语 `scrubPayload`（R1–R4）** (#348)：新增导出 `scrubPayload`＝`slice → redactField → escapeBlock → redactField(+MARK) → slice → trimTrailingSecretPrefix → containsAnySecret?''`。字符串面 `describeFailure`、对象面 `describeFailureStack`、盘文本面（`webhook-delivery` 两处 corrupted delivery log）**全部改调它**；grep 断言禁止 `escapeLine(redactSecrets(...))` 手工直连。R4 兜底＝空串（不迭代替换）。上界 6002 / 8204；Codex P2 孤立反斜杠收尾。对象面 6 处 `console.error(..., err)` 改走 `describeFailureStack`；#347 残余白名单不变。**`errorCode` 一字不动；对外错误码/状态码/body 形状零变更**。设计：`/home/ops/materials/log-face-r5/design.md`。**合并 ≠ 生效，生效于下次部署窗**。
