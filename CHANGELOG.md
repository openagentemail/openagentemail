...
- **API/MCP: `@modelcontextprotocol/server` 2.0.0→2.1.0 受控兼容评估** (#357)：两包锁 `^2.1.0`（bun.lock 钉 2.1.0）；接受上游默认 **4MiB** `maxRequestBodySize`（超限 413，贴限下大件仍 200）。**DPoP 未验证、未启用**（我方不实现 client）。**scopeChallenge 未采用**（critical 工具对 OAuth 403 仍由自有 tier 层等价覆盖）。订阅/listen 面未使用。合并 ≠ 生效，生效于下次部署窗。
