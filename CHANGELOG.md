...
- **API: 对象面日志统一为 `describeFailureStack`（保 stack + 有界 + 脱敏）** (#344)：新增唯一对象面入口——取 `err.stack` → 内层守卫 8192 → `redactField` → `escapeBlock` → **再 `redactField`（吞转义生成的密钥字面）** → 外层截断至 **STACK_MAX=8192** → 尾部真前缀回退 → `…[truncated]`。可证上界 **≤8204**。6 处原 `console.error(..., err)` 改走该入口。**`errorCode` 一字不动；对外错误码/状态码/body 形状零变更**。**本卡表列 6 处已收口**；其余对象面裸用记债 **#347**。设计：`materials/obj-face-344/r0.md`。**合并 ≠ 生效，生效于下次部署窗**。
