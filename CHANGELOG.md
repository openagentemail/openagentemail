...
- **API: scrubPayload 余债收口（失败链线性化 / 单码元转义钥 / 测试卫生）** (#350)：`redactField` 改 AC 风格失败链（禁自重叠病态整段 pending 回放）；尾退从长到短命中；`prepareSecretEscapePreps` 收录单码元转义形态；测试临时目录改 `mkdtempSync(tmpdir)`。**D（R4 空串兜底不可区分）本卡不修、只记档**（可观测须在调用方，与唯一原语相悖）。**`errorCode` 一字不动；对外错误码/状态码/body 形状零变更**。**合并 ≠ 生效，生效于下次部署窗**。
