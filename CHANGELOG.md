...
- **API: 日志面发射路径收敛为唯一原语 `scrubPayload`（R1–R4）** (#348)：新增导出 `scrubPayload`（显式 `mode ∈ {line,block}`）及薄封装 `scrubLinePayload` / `scrubBlockPayload`。串面 `describeFailure`→**line**（`\n`/`\r`/`\t` 转义，单行不变量）；对象面 `describeFailureStack`→**block**（保换行）；盘文本默认 **line**（磁盘行防日志注入）。R4 空串兜底；对象面输入界＝8204−|MARK|，`needsMark`＝输入||输出截断。上界 6002/8204。对象面裸用扫描**按参数判定**（删整段 `describeFailure*` 豁免捷径，防混合调用绕过）。**`errorCode` 一字不动；对外错误码/状态码/body 形状零变更**。不修：#342 已声明的串面静默截断/J4 尾丢；R4 空串不可区分记观察。设计：`/home/ops/materials/log-face-r5/design.md`。**合并 ≠ 生效，生效于下次部署窗**。
