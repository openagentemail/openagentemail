...
- **API: 日志面发射路径收敛为唯一原语 `scrubPayload`（R1–R8）** (#348)：新增导出 `scrubPayload`（显式 `mode ∈ {line,block}`）及薄封装。串面 `describeFailure`→**line**；对象面 `describeFailureStack`→**block**；盘文本默认 **line**。R4 空串兜底；R5 `needsMark` 二遍脱敏后重算；R6/R7 裸用扫描含 `String(errish)` / `[.|?.]message|stack`；R7 boot reconstruction warn→`describeFailure`。**R8**：转义后 ④ 脱敏与 ⑦ 完整钥检查用 **原文∪escape(密钥)**；尾部回退覆盖 **escape(真前缀)** 及「剥 `\\n` 后露出原文真前缀」形态（修 CR：口令含真实换行时尾 `ab\\n` 静默残留）。**`errorCode` 一字不动；对外错误码/状态码/body 形状零变更**。设计：`/home/ops/materials/log-face-r5/design.md`。**合并 ≠ 生效，生效于下次部署窗**。
