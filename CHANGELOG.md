...
- **API: 对象面日志统一为 `describeFailureStack`（保 stack + 有界 + 脱敏）** (#344)：新增唯一对象面入口——取 `err.stack`（非空串）→ 有界 **STACK_MAX=8192** → `redactField`（**整段 stack 单域**，密钥含 `\n` 亦可匹配）→ `escapeBlock`（与 `escapeLine` **同源转义判据**，但保留 `\n`/`\r`/`\t` 字面）→ 截断则尾部追加 `…[truncated]`（脱敏之后）。6 处原 `console.error(..., err)` 改走该入口（`app` / `main` / `webhook-delivery`×3 / `audit`）。**`errorCode` 一字不动；对外错误码/状态码/body 形状零变更**（本卡只动日志面）。**#342 对象面 6 处豁免清零**（I6/J8 零豁免）。设计：`materials/obj-face-344/r0.md`。**合并 ≠ 生效，生效于下次部署窗**。
