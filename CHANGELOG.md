...
- **API: 日志面发射路径收敛为唯一原语 `scrubPayload`（R1–R7）** (#348)：新增导出 `scrubPayload`（显式 `mode ∈ {line,block}`）及薄封装 `scrubLinePayload` / `scrubBlockPayload`。串面 `describeFailure`→**line**（`\n`/`\r`/`\t` 转义，单行不变量）；对象面 `describeFailureStack`→**block**（保换行）；盘文本默认 **line**（磁盘行防日志注入）。R4 空串兜底；对象面输入界＝8204−|MARK|；**R5：`needsMark` 在第二遍脱敏探针之后按最终长度重算**。上界 6002/8204。对象面裸用扫描**按参数判定**（**R6/R7**：`String(errish)` / `errish[.|?.]message|stack` / `(errish as …)[.|?.]message|stack`）。**R7**：`webhook-delivery` boot reconstruction transient warn 由 `err?.message` 改走 **`describeFailure(err)`**（串面一行，与 sent-registry warn 同族；触发条件与 retry 流程不动）。**`errorCode` 一字不动；对外错误码/状态码/body 形状零变更**。不修：#342 已声明的串面静默截断/J4 尾丢；R4 空串不可区分记观察。设计：`/home/ops/materials/log-face-r5/design.md`。**合并 ≠ 生效，生效于下次部署窗**。
