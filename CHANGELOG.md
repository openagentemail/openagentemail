...
- **API: 对象面日志统一为 `describeFailureStack`（保 stack + 有界 + 脱敏）** (#344)：新增唯一对象面入口——取 `err.stack`（非空串）→ **内层守卫**截 8192 → `redactField`（**整段 stack 单域**）→ `escapeBlock`（与 `escapeLine` **同源转义判据**，保留 `\n`/`\r`/`\t` 字面）→ **外层截断至 STACK_MAX=8192（最终输出上限，覆盖转义/脱敏膨胀）** → 截断则尾部 `…[truncated]`。可证上界 **≤8204**。6 处原 `console.error(..., err)` 改走该入口（`app` / `main` / `webhook-delivery`×3 / `audit`）。**`errorCode` 一字不动；对外错误码/状态码/body 形状零变更**（本卡只动日志面）。**本卡表列 6 处已收口**；全仓其余对象面裸用记债 **#347**（非「全仓零裸用」）。设计：`materials/obj-face-344/r0.md`。**合并 ≠ 生效，生效于下次部署窗**。
