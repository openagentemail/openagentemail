...
- Even with journal off, `claim` returns **409 `lease_overlay_pending_index`** while a `release`/`renew` is SMTP-accepted but not yet durable-indexed; retry after absorption (#308).
