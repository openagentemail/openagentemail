...
/**
 * 模块级串行化：后一次 write 等前一次完成后再拍 agents 快照并落盘，
 * 避免连续 deleteIdentity 时先删的 stale 快照晚写回复活幽灵 reader。
 */
let writeServerConfigTail: Promise<void> = Promise.resolve();
...
/** @internal 测试缝：等待 writeServerConfig 队列排空。 */
export async function flushWriteServerConfigForTests(): Promise<void> {
  await writeServerConfigTail;
}
...
async function writeServerConfig(state: NotifyState): Promise<void> {
  const run = async (): Promise<void> => {
    ...
    // 快照必须在拿到队列槽位之后拍：此时先前写已完成，agents 为当前终态。
  ...
  const next = writeServerConfigTail.then(run, run);
  // 链本身不因单次失败断裂，后续写仍可排队。
  writeServerConfigTail = next.then(
    () => undefined,
    () => undefined,
  );
  return next;
