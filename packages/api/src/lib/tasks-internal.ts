...
type QueuedEvent = {
  ...
  sentAt: number;
  lease?: LeaseEvent;
  /** 历史窗补账：只跟踪索引进度，不进公共 messages / updatedAt。 */
...
function applyOverlayMessages(task: Task, extra: QueuedEvent[]): Task {
  // 补账回执不进公共 overlay，避免索引前幻影 "Lease expired." 与 updatedAt 抖动。
