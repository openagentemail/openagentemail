...
| 折行后线上行数 | 视 MTA 折行宽 | 同左 | 同左，显著更多 | 生产解析（lease + approval payload + **task-root + approval-digest**）**接受 MIME 中介重折**（strip 空白后再严格 round-trip / hex 校验） |
...
4. 回归：`packages/api` 内 `task-lease-core` 对 8k reason 的 folding/unfolding
   ...
   生产 `readLeaseEventPayload`、`readApprovalPayloadHeader` 与 `readTaskRootEnvelope`
   （及 approval-digest 头值 compact）**均接受 MIME 中介重折**：
   对头值 strip 全部空白后再做严格 base64url round-trip（root/payload）或 hex 校验（digest）。
   安全性：base64url / hex 字母表不含空白，删除无歧义；非法非空白字符（如 `!!!!`）strip 后仍拒——容忍重折 ≠ 容忍垃圾。
   **边界（其余短头不补）**：枚举/ID 头（Task / State / Event / Approval-Event / Decision / Lease-Event）
   行宽极窄，需极窄折才失败且 fail-closed；#4 Stamp（60 列）strip 会触 5 个 HMAC 域语义、收益低；
   #13 Mail-Stamp 失败=安全降级 external。**#12 Idempotency-Key**：键可合法含空白、strip 不可行——
   写侧约束已另立记债 **#314**（本卡不做写侧）。
