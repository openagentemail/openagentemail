...
**只读投影，不做后台物化器。** list / board（`toTaskView` / `toUiTaskView`）对仍停在 `input-required`、`expiresAt` 已按 server clock 到期（与 authorized detail/wait/decision 相同的 `now >= expiresAt` 边界）、且尚无 signed `expired` 事件的审批，派生只读字段 `expiryProjection: "past-deadline-unmaterialized"`。计算纯读、零写副作用：不物化 signed expiry、不发信。authorized `get` / `wait` / `decision` 物化后该字段消失。未认证与 list/board 路径不得借投影引入写副作用（#80/#84 的写面教训）。Dashboard board / Home / 缓存详情把该字段呈现为 Past deadline，且 `approvalCanDecide` 为 false；不得再标成 Waiting for you。Home 的 Waiting for you 从 `input-required` 结果里滤掉该投影行（计数从 `totalApprox` 扣本页投影行），并入 Blocked。同一 TaskView 的下游严格校验器（MCP `taskOutputSchema`、`examples/adapters` `isValidTaskView`）必须把该可选字面量列入白名单（#133 additionalProperties 教训）。
