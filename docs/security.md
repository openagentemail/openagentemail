...
- **票 = 父身份本人全能力（语义钉，#317）**：`/v1` 按 `kind:'identity'` 处理并继承身份当前落盘的 scopes（`lib/auth.ts:172-178`）；**含以归属子地址发信**（`routes/send.ts` `allowParentSendAsChild` 不查 attribution）**与读子信箱**（`lib/auth.ts` `forbidUnlessMailboxAccess` 归属分支不区分 attribution）——「OAuth 视同父身份本人」为**有意决策**：票本就持有父全部 REST 权限（含以父地址发信），父子同域同声誉，不构成权限等级跃升。**不对称边界同为有意**：「造凭证的面」——子身份创建（`routes/identities.ts:241/:290`）与 delegation 管理（`routes/delegations.ts:89/:247`）——显式拒 OAuth 票（requires direct identity credentials）；「用凭证的面」（发信/读子归属）放行。**变更闸**：将来任何「更窄的票」设计与本语义正面冲突，须先经显式裁决（RFC/业主拍板），不得静默收窄；本节为该语义唯一钉。
- **子身份 50 配额为软上限（#317 声明）**：`countChildren` 检查非 CAS（`routes/identities.ts:332`），并发创建可短暂越过 `MAX_CHILD_IDENTITIES`——已裁定接受（越界量级无害）；若将来要硬上限须换原子计数，另卡另裁。
...
| 级别 | 工具 | 策略要点 |
| --- | --- | --- |
| read | mail_list_messages, mail_read_message, mail_wait_for, mail_list_identities, mail_webhook_list, notify_check, task_list, task_get, task_list_children | 观测 |
| minimal | mail_mark_seen, mail_webhook_delete, mail_webhook_disable, task_create | 轻状态变更 |
| contained | mail_send, mail_webhook_test, task_update, task_decide, task_claim, task_renew, task_release, notify_agent, notify_user | 外发 / 唤醒 |
| critical | mail_new_identity, mail_webhook_create, notify_verify | **OAuth 票 deny-by-default（403）**；`oa_` 走 REST scope；admin 全通 |

这份 25 tools 清单取代先前的 20 tools 清单；三项 task lease 调用均为 `contained`，直接子任务查询 `task_list_children` 为 `read`。
